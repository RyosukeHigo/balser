...
#define GBRG 3
	...
	void _bayer2rgb_wb_8u(const unsigned char* _src, unsigned char* _dest, unsigned int _width,
					   unsigned int _pitchs, unsigned int _pitchd,
					   int _start, int _stop, 
					   short rcoeff, short gcoeff, short bcoeff) {
...
//		_start += 1; _stop -= 1;
		...
		for( int y=_start; y<_stop; y+=2 ) {
			...
			bool rhas_high_bit = rcoeff > 255;
			bool ghas_high_bit = gcoeff > 255;
			bool bhas_high_bit = bcoeff > 255;
#ifdef USE_SSE
			for( ; x<_pitchs-16; x+=14 ) {
				...
				if( LAYOUT == RGGB || LAYOUT == BGGR ) {
					...
					__m128i vrt = _mm_srli_epi16(add_t, 2);
					// Red adjusted
					__m128i reven = _mm_srli_epi16(vr,1);
					__m128i rodd = _mm_and_si128(_mm_set1_epi16((short)0xFF),vrt);
					__m128i reven_adj = _mm_srli_epi16(_mm_mullo_epi16(reven,_mm_set1_epi16(rcoeff)),8);
					__m128i rodd_adj = _mm_and_si128(_mm_set1_epi16((short)0xFF00),_mm_mullo_epi16(rodd,_mm_set1_epi16(rcoeff)));

					// Red combined
					vr = _mm_srli_si128(_mm_add_epi8(reven_adj,rodd_adj),1);
					if( rhas_high_bit ) {
						vr = _mm_adds_epu8(vr,_mm_add_epi8(reven,_mm_slli_epi16(rodd,8)));
					}
					...
					// Green adjusted
					__m128i geven_adj = _mm_srli_epi16(_mm_mullo_epi16(vg,_mm_set1_epi16(gcoeff)),8);
					__m128i godd_adj  = _mm_and_si128(_mm_set1_epi16((short)0xFF00),_mm_mullo_epi16(vrt,_mm_set1_epi16(gcoeff)));
					// Green combined
					vg = _mm_srli_si128(_mm_add_epi8(geven_adj,godd_adj),1);
					...
					// Blue adjusted
					__m128i beven_adj = _mm_srli_epi16(_mm_mullo_epi16(_mm_slli_si128(vrt,2),_mm_set1_epi16(bcoeff)),8);
					__m128i bodd_adj  = _mm_and_si128(_mm_set1_epi16((short)0xFF00),_mm_mullo_epi16(_mm_srli_si128(v2b, 1),_mm_set1_epi16(bcoeff)));
					
					__m128i vb = _mm_srli_si128(_mm_add_epi8(beven_adj, bodd_adj),1);
				...
				{ // THIS BLOCK ALWAYS BEGINS WITH RED OR BLUE
					...
					__m128i vrt = _mm_srli_epi16(add_t, 1);
					// Red adjusted
					__m128i reven_adj = _mm_srli_epi16(_mm_mullo_epi16(vr,_mm_set1_epi16(rcoeff)),8);
					__m128i rodd_adj  = _mm_and_si128(_mm_set1_epi16((short)0xFF00),_mm_mullo_epi16(vr,_mm_set1_epi16(rcoeff)));
					// Red combined
					vr = _mm_srli_si128(_mm_add_epi8(reven_adj,rodd_adj),1);
					...
					vrt = _mm_srli_epi16(add_t, 2);
					// Green adjusted
					__m128i geven_adj = _mm_srli_epi16(_mm_mullo_epi16(vrt,_mm_set1_epi16(gcoeff)),8);
					__m128i godd_adj  = _mm_and_si128(_mm_set1_epi16((short)0xFF00),_mm_mullo_epi16(_mm_srli_si128(vg,1),_mm_set1_epi16(gcoeff)));
					// Green combined
					vg = _mm_srli_si128(_mm_add_epi8(geven_adj,godd_adj),1);
					...
					// Blue adjusted
					__m128i beven_adj = _mm_srli_epi16(_mm_mullo_epi16( _mm_slli_si128(vrt,2),_mm_set1_epi16(bcoeff)),8);
 					__m128i bodd_adj  = _mm_and_si128(_mm_set1_epi16((short)0xFF00),_mm_mullo_epi16(_mm_srli_epi16(vb, 1),_mm_set1_epi16(bcoeff)));
					// Blue combined
					vb = _mm_srli_si128(_mm_add_epi8(beven_adj,bodd_adj),1);
...
//			if( stop != height ) stop += 1;
			...
			_bayer2rgb_wb_8u<LAYOUT,HAS_ALPHA>( src, dest, width, pitchs, pitchd, start, stop, rcoeff, gcoeff, bcoeff );
		}
		
		BAYER2RGB_WB_8U_TBB( const unsigned char* _src, unsigned char* _dest,
						 unsigned int _width, unsigned int _height,
						 unsigned int _pitchs, unsigned int _pitchd, 
						 short _rcoeff, short _gcoeff, short _bcoeff,
						 unsigned int _threads ) :
		src(_src), dest(_dest), width(_width), height(_height), pitchs(_pitchs), pitchd(_pitchd), 
			rcoeff(_rcoeff), gcoeff(_gcoeff), bcoeff(_bcoeff), threads(_threads) {
